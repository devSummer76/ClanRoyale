...
export function cmdDeserters(msg: Discord.Message, clanTag: string, ROYALE_API_TOKEN: string) {
    var options = {
        method: 'GET',
    ...
    request.get(options, function (error, response, body) {
        ...
        for (var i in warlog) {
            ...
            if (!clanBadgeUrl) clanBadgeUrl = myClanStanding.badge.image;
            var warDeserters = "";
            war.participants.filter(player => player.battlesPlayed == 0).forEach(function (des) {
        ...
        msg.channel.send({ embed })
            ...
            .catch(console.error);
    }).on('error', function (e) { // TODO: seems not to be called on error (e.g. bad api token)...
